...
## script for looping through B. napus bam files
## from sorted bam --> mapped fastq + consensus fasta + vcf
...
## save file
#"AA_Chr08:13383605-13385125"

# save file
#write.table(genecoord, file = paste0(args[1], ".txt", sep = "\n"))
write.table(genecoord, file= paste(ID, ".txt"))
######################################################

## working directory (original):
#"/mnt/lustre/groups/bio-pgen-2019/kg913/Brassica/scripts"
...
##########################################
## create folder with logs + scripts
...
for(i in 1:length(exp.condition)){
  ...
  # run second script - submit-script
  #cat("Rscript submit-script.R ", ID, " \n",
  #    file = command1, append = T, sep = "")
...
# loop to run .sh scripts

# directory: mnt/lustre/groups/biol-pgen-2019/kg913/Brassica/scripts

# get gene ID from previous script
args <- commandArgs(T)
cat(args, sep = "\n")
ID <- args[1] # ID = gene ID (eg. Cab020592.1)

# note: submit-script.sh needs to be temporarily moved to ID/sh folder
...
for (i in 1:length(scripts)){
  command1 <- paste("submit-script.sh", sep="")
